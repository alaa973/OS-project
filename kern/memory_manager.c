...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	int numOfPages = ROUNDUP(size, PAGE_SIZE) / PAGE_SIZE ;
	uint32 vr=virtual_address;
	unsigned int start = virtual_address;
	for (int i =0 ; i < numOfPages ; i++ )
	{
		//arr[i] = vr;
		pf_remove_env_page(e, vr);
		vr+= PAGE_SIZE;
	...
	LIST_FOREACH(element,&(e->ActiveList)){
		if(element->virtual_address<(virtual_address+size) && element->virtual_address>=virtual_address){
			...
			struct Frame_Info *page_table_frame;
			uint32* ptr_page_table = NULL;
			uint32* page_table = NULL;
			int t = 0;
			frame = get_frame_info(e->env_page_directory,(void*)element->virtual_address,&ptr_page_table);
			page_table_frame = get_frame_info(e->env_page_directory,(void*)ptr_page_table,&page_table);
			unmap_frame(e->env_page_directory,(void*)element->virtual_address);get_page_table(e->env_page_directory,(void*)element->virtual_address,&ptr_page_table);
			if(ptr_page_table !=NULL){
				...
				if(t==1024){
					free_frame(page_table_frame);
					page_table_frame->references=0;
			...
			if(LIST_SIZE(&(e->SecondList))>0){
				struct WorkingSetElement *elementt = LIST_FIRST(&(e->SecondList));
				LIST_REMOVE(&(e->SecondList), elementt);
				LIST_INSERT_TAIL(&(e->ActiveList), elementt);
				pt_set_page_permissions(e, elementt->virtual_address, PERM_PRESENT, 0);
			}
		}
	}
	struct WorkingSetElement* elem;
	LIST_FOREACH(elem,&(e->SecondList)){
		if(elem->virtual_address<(virtual_address+size) && elem->virtual_address>=virtual_address){
			struct Frame_Info *frame;
			struct Frame_Info *page_table_frame;
			int t=0;
			uint32* ptr_page_table = NULL;
			uint32* page_table = NULL;
			frame = get_frame_info(e->env_page_directory,(void*)elem->virtual_address,&ptr_page_table);
			page_table_frame = get_frame_info(e->env_page_directory,(void*)ptr_page_table,&page_table);
			...
			if(ptr_page_table !=NULL){
				...
				if(t==1024){
					free_frame(page_table_frame);
					page_table_frame->references=0;
					pd_clear_page_dir_entry(e,(uint32)elem->virtual_address);
				}
			}
			LIST_REMOVE(&(e->SecondList),elem);
			LIST_INSERT_TAIL(&(e->PageWorkingSetList),elem);
		}
	}
